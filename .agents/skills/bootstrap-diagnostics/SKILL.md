...
- `MISSING: <tool> (install: <command>)` - list the missing tools to the captain with a one-line purpose each plus the printed install commands, wait for consent (one approval may cover the list), then run `bin/fm-bootstrap.sh install <approved tools...>`.
  ...
  For `gh`, this covers a completely absent `gh`, which stays an automated install; only an installed `gh` below its floor is reported as `MISSING_MANUAL` instead.
...
- `MISSING_MANUAL: <tool> (instructions: <url>)` - tell the captain why the tool is required and give them the printed instructions URL, but do not pass the tool to `bin/fm-bootstrap.sh install`; wait for the captain to complete the manual installation, then rerun session start to confirm the dependency is present.
  For `gh`, this is the below-floor case ([`bin/fm-bootstrap.sh`](../../../bin/fm-bootstrap.sh) owns the floor and why it exists): `gh` usually came from a platform package manager or installer, so the upgrade is the captain's to run.
...
- `ORPHAN_PANE: session=<s> pane=<id> cwd=<path|missing> reason=<cwd-missing|dead-agent|idle-untracked> title=<title> [slot=<name> slot_status=<status>]` - `bin/fm-orphan-pane-sweep.sh` found a herdr pane outside every firstmate home's task tracking with a strong orphan signal.
  This is detection only: nothing was closed. Inspect the named pane (`herdr pane get <session>:<pane-id>`, or attach the session and look), confirm it is genuinely stray - not a legitimate personal or in-progress pane the sweep could not attribute - and close it by hand (`herdr tab close`) only after that confirmation; never close on the line alone.
  A `cwd-missing` reason is the strongest signal (its working directory is gone, so nothing legitimate can be running there); `dead-agent` and `idle-untracked` warrant the same manual look before closing.
...
- `CREW_DISPATCH: unverified config/crew-dispatch.json - jq could not evaluate the validity contract` - the file exists but `jq` failed to run the validity filter, so the config is neither confirmed valid nor invalid; treat it as untrustworthy for profile-based dispatch and fix the `jq` installation before relying on it.
...
- `BOOTSTRAP_INFO: returned <id> to the queue held as parked after interrupted cleanup; its endpoint may remain and should be reconciled` - replay parked the item, but the park's endpoint cleanup was interrupted.
  Verify process reaping and endpoint closure, then reconcile any surviving endpoint; never return or reset the retained local copy the item's body names.
