...
- Never pass data that grows with fleet or backlog size through `jq` argv (`--arg`/`--argjson`), because the whole value lands in the exec argument list and fails with `E2BIG` once it is large enough; feed it on stdin or from a temp file via `--slurpfile`/`--rawfile` instead, as `json_tmpfile` in `bin/fm-fleet-snapshot.sh` does and `tests/fm-fleet-snapshot-view.test.sh` guards at scale.
