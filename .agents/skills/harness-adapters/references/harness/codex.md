...
A raw launch firstmate cannot read conclusively is refused outright rather than classified as another harness: an unterminated quote, a newline, a command word that is a shell expansion, tilde, glob or brace pattern, a command substitution that is unquoted or double-quoted (inside single quotes it is literal text, exactly as bash treats it), an unquoted control operator or redirection, or no command word at all.
