...
Run `bin/fm-questionnaire-refill-source.sh` (14-day staleness ceiling): it owns candidates-first - reads the newest matching report over `data/*roundtable*/report.md` and `data/*roadmap*/report.md` regardless of age, dispatches a fresh sweep only if absent or its candidates are already spent, and enforces a 14-day hard staleness ceiling.
