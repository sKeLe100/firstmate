...
2. **Open with the availability/role hierarchy header, before the numbered list.**
   ...
   When `hierarchy_lanes: unavailable (dispatch_config: <status>)`, say once that no lane hierarchy can be shown for that reason (the same absent/invalid/unverified explanation used for per-item tiers below) and skip straight to `live_slots`.
   ...
   Render `live_slots` as the count of currently tracked work; firstmate has no configured concurrency cap by default, so state that plainly rather than inventing or asking about a cap that may not exist.
