...
Before pinging or resuming a LOCAL secondmate that is already running - never before a fresh respawn of a genuinely dead one - run `bin/fm-returning-session-check.sh <secondmate-home> "$STATE" <id>` to read its last known context level, passing this firstmate's own `state/` (where that secondmate's `state/<id>.meta`, `.status`, and `.turn-ended` live) so the check also folds in idle time.
