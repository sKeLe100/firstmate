...
Every convergence point (local propagation, `bin/fm-config-push.sh`, and the remote sender/receiver) checks that marker before touching `crew-harness` and, when present, leaves the item alone in both directions instead of re-pushing the primary's value or mirroring its absence; `fm_config_inherit_item_overridden` in [`fm-config-inherit-lib.sh`](../../../bin/fm-config-inherit-lib.sh) is the single owner of the check, and `FM_LOCAL_OVERRIDABLE_CONFIG` there is the one declared list of items eligible for this override (`crew-harness` today).
