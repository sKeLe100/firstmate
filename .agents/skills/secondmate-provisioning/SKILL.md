...
A raw launch command is not available to a secondmate spawn on either route: local secondmate spawns refuse it outright, and a remote route accepts only a verified harness adapter at the host boundary.
...
A secondmate home may instead pin its own `config/crew-harness` independent of the primary - e.g. so a Codex secondmate's crewmates spawn on Codex regardless of what the primary is pinned to - by writing that file locally and also leaving a sibling `config/crew-harness.local-override` marker (empty file, presence-only, content never read) next to it.
Local propagation (`bin/fm-config-push.sh` and the launch/bootstrap sweeps included) checks that marker before touching `crew-harness`, and on the remote path the receiver `bin/fm-remote-inherit.sh` enforces it - the remote sender still transfers `crew-harness` on every push and the receiver is what skips the write or removal; when the marker is present the item is left alone in both directions instead of taking the primary's value or mirroring its absence. `fm_config_inherit_item_overridden` in [`fm-config-inherit-lib.sh`](../../../bin/fm-config-inherit-lib.sh) is the single owner of the check, and `crew-harness` is the only item eligible for it; a marker path that is not a regular file is reported and ignored.
Removing the marker resumes ordinary primary-authoritative inheritance for that item at the next convergence.
This is the only declared exception to primary authority in this contract; every other inheritable item still always converges to the primary's value or absence.
...
Before pinging or resuming a LOCAL secondmate that is already running - never before a fresh respawn of a genuinely dead one - run `bin/fm-returning-session-check.sh <secondmate-home> <firstmate-home>/state <id>` to read its last known context level, passing this firstmate's own `state/` (where that secondmate's `state/<id>.meta`, `.status`, and `.turn-ended` live) so the check also folds in idle time.
