...
The same placement-specific launch and deferred bootstrap sweep also propagate the primary's declared inherited local material: every `config/` item in `FM_INHERITABLE_CONFIG` (`bin/fm-config-inherit-lib.sh` owns that declared set), plus the one shared captain-preference file `data/captain-shared.md`.
