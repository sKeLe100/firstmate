...
Pass the worktree's absolute path, and treat a non-zero exit with `verdict=blocked` as a broken `config/context-thresholds` in that home to fix before resuming, never as permission to bare-resume.
