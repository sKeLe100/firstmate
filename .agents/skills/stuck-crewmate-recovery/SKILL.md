...
Before resuming or relaunching that recorded worktree - a returning session, never a fresh spawn - run `bin/fm-returning-session-check.sh <task-worktree-path> <firstmate-home>/state <task-id>` to read its last known context level, passing this firstmate's own `state/` (where `state/<task-id>.meta`, `.status`, and `.turn-ended` live, not the worktree) so the check also folds in idle time.
A `verdict=restart-with-carryover` line means a bare resume would pay the full return-tax re-read straight back into the same restart band - either because context alone is already there, or because the session has sat idle past its cache TTL at warn band and would land there on the very next turn anyway: relaunch with a carryover note (`bin/fm-control.sh <task-id> relaunch --note "carryover: what is done, what is live, exact pickup commands"`) instead of a plain resume, exactly as docs/configuration.md "Session context thresholds" already prescribes for that band.
...
When reconciling a task whose last status line records `paused [key=session-limit]: {harness} session usage limit, resets <ts>` (a crewmate hitting its own Claude session usage limit, distinct from account-wide quota exhaustion or a real wedge) and the limit has since reset: if the task's context is past the "warn" band (read via `bin/fm-context-usage.sh` under that task's worktree as `FM_HOME`), relaunch with a carryover note (`bin/fm-control.sh <task-id> relaunch --note "carryover: what is done, what is live, exact pickup commands"`) rather than assuming the existing session will or should continue. Do not simply leave a session-limited pause to clear on its own once the reset time has passed and context is large, because the >1h idle since the stall guarantees the prompt cache has gone cold and a continuing session would pay a full cold re-cache at that session's model rate for no new work. `bin/fm-returning-session-check.sh <task-worktree-path> <firstmate-home>/state <task-id>` now folds this idle dimension into its own verdict (a warn-band session idle past its TTL reads `restart-with-carryover` too), so run it the same way as above rather than re-deriving the >1h check by hand.
