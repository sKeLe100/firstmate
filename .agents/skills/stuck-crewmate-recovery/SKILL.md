...
Before resuming or relaunching that recorded worktree - a returning session, never a fresh spawn - run `bin/fm-returning-session-check.sh <task-worktree-path> "$STATE" <task-id>` to read its last known context level, passing this firstmate's own `state/` (where `state/<task-id>.meta`, `.status`, and `.turn-ended` live, not the worktree) so the check also folds in idle time.
