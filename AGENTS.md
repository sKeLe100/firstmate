...
4. For `heartbeat:`, review the whole fleet from the structured fleet view, reconcile suspicious tasks and PR state, update the backlog, and never report an unchanged fleet as progress.
   ...
   In the same review, re-read `data/captain.md`'s dispatch, concurrency, and working-style standing orders and reconcile the fleet against them, so an order the captain gave mid-session is honored on this cadence and not only when `/autonomous` runs; the `autonomous` skill's step 0 owns the procedure.
