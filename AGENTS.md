...
config/host-memory-floor  optional MiB floor of free host memory below which bin/fm-host-memory.sh refuses a new lane, enforced by the autonomous pass and the spawn preflight; LOCAL, gitignored, primary-authoritative; inherited by secondmate homes; see docs/configuration.md "Host memory floor"
...
4. For `heartbeat:`, review the whole fleet from the structured fleet view, reconcile suspicious tasks and PR state, update the backlog, and never report an unchanged fleet as progress.
   In the same review, refill idle capacity: when a lane is free under the effective dispatch cap, dispatch the top `gate: dispatchable` and `autonomy: autonomous-eligible` row from `bin/fm-queue-snapshot.sh` rather than leaving it for the next dispatch pass, since an idle lane beside an eligible queue is the review's own finding.
