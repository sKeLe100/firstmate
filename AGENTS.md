...
After the profile is resolved, a scout or ship intake whose purpose class is review, investigation, or planning also resolves an optional worker perspective through `perspective-catalog`, which owns the catalog, precedence, and purpose-class defaults; `bin/fm-brief.sh --perspective` inserts it.
...
- `perspective-catalog` - load at every scout or ship intake whose purpose class is review, investigation, or planning, after the dispatch profile is resolved and before scaffolding the brief, and before editing a catalog fragment.
