...
After the profile is resolved, a scout intake whose purpose class is review, investigation, or planning also resolves an optional worker perspective through `perspective-catalog`, which owns the catalog, precedence, and purpose-class defaults; `bin/fm-brief.sh --perspective` inserts it.
`bin/fm-brief.sh` refuses `--perspective` on a ship brief.
...
- `perspective-catalog` - load at every scout intake whose purpose class is review, investigation, or planning, after the dispatch profile is resolved and before scaffolding the brief, and before editing a catalog fragment.
