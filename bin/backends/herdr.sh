...
# creating a second Herdr state machine: a stopped server or a structurally
# gone pane is `missing`, a confirmed agent-less pane (including a registered
# agent whose process the shutdown killed, proven via the pane's own bare
# idle shell) is `dead`, a registered agent is `alive`, and an unexpected or
...
fm_backend_herdr_agent_state() {  # <target>
  ...
  # this without needing a live connection to succeed (`// false` already
  # covers a failed or empty read), so check it first and treat a down
  # server as authoritatively missing - the same recovery branch a
  # structurally absent pane takes.
  running=$(fm_backend_herdr_cli "$FM_BACKEND_HERDR_SESSION" status --json 2>/dev/null | jq -r '.server.running // false' 2>/dev/null)
  [ "$running" = true ] || { printf 'missing'; return 0; }
