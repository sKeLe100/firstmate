...
fm_crew_dispatch_validate() {
  ...
  err=$(jq -r --argjson codex_efforts "$codex_efforts" '
    ...
    def effort_ok($h; $m; $e):
      ...
      elif $h == "codex" then (($codex_efforts | index($e)) != null or ($e == "max" and $m == "gpt-5.6-luna"))
    ...
    def malformed_optional_fields($items):
      ...
      or ($items | any(has("perspective") and (((.perspective | type) != "string") or (.perspective | length) == 0)));
    def bad_efforts:
      configured_profiles
    ...
    elif malformed_optional_fields([(.rules // [])[]? | profiles(.use?)[]?]) then "use profile model, effort, and perspective must be non-empty strings when present"
    ...
    elif has("default") and malformed_optional_fields([profiles(.default)[]?]) then "default profile model, effort, and perspective must be non-empty strings when present"
    else
      (configured_profiles
        | map(.harness)
        | map(select(. != null))
