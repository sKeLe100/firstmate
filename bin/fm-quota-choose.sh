...
# per-window pace evidence, which only the `--json` snapshot carries. The TOON
# row carries a per-scope confidence (field 5) instead: a value of `unknown`
# there flags an unmeasurable scope. The first eligible
