...
#   Ultra is the other explicit exception: bin/fm-harness.sh validate-native-effort
#   owns its model scope; supported Pi launches receive --codex-effort ultra,
#   never --thinking ultra.
...
for a in "$@"; do
  if [ -n "$want_value" ]; then
    ...
    redelegated-from)
      REDELEGATED_FROM=$a
      REDELEGATED_FROM_SET=1
      ;;
    redelegation-reason)
      REDELEGATION_REASON=$a
      REDELEGATION_REASON_SET=1
      ;;
    *)
  ...
  --redelegated-from=*)
    REDELEGATED_FROM=${a#--redelegated-from=}
    REDELEGATED_FROM_SET=1
    ;;
  --redelegation-reason) want_value=redelegation-reason ;;
  --redelegation-reason=*)
    REDELEGATION_REASON=${a#--redelegation-reason=}
    REDELEGATION_REASON_SET=1
    ;;
...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  [ "$PURPOSE_SET" -eq 0 ] || {
    echo "error: --relaunch carries the task's recorded purpose forward; --purpose cannot override it" >&2
    exit 1
  }
  [ "$MODE_SET" -eq 0 ] || {
...
launch_template() {
  ...
  cursor) printf '%s' 'env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT -u FM_PI_HARNESS -u GEMINI_CLI -u CURSOR_INVOKED_AS -u FM_CREW_STATE_META_OVERRIDE -u FM_CREW_STATE_STATUS_OVERRIDE __CURSORBIN__ --trust --yolo __MODELFLAG__--workspace __WORKTREE__ "$(__OPINPUT__ encode launch-brief < __BRIEF__)"' ;;
...
if [ "$HARNESS" = codex ] && [ "$RAW_LAUNCH" -eq 0 ]; then
  ...
  if [ -z "$(effort_flag_for_harness codex "$EFFORT" "$MODEL")" ]; then
