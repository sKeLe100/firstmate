...
1. **Resolve the profile.**
   ...
   A resolved profile that leaves codex with no model, or an effort codex does not accept, is refused here rather than after the checkpoint, because codex would otherwise launch on Codex CLI's own billed default ([architecture.md](architecture.md#dispatch-profiles)).
2. **Safe checkpoint.**
