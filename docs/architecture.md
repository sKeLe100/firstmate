...
Upstream/main separately added a pane-churn-absorb evidence mechanism (`config/turnend-churn-absorb`, `signal_turnend_panes_churned`, `state/.churn-since-*`) in this same triage path; this home intentionally kept the process-event coverage-deferral mechanism above instead of adopting it in the 2026-08-31 upstream sync.
