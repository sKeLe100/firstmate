...
A wedged CI poll - the same CI check command failing identically several times with no forward-progress marker after its last occurrence, where a per-poll heartbeat interleaved with those failures does not count as progress - overrides those markers and reports the crew failed, so a poll that can never progress is never read as still monitoring; the coarse cross-branch fallback is not wedge-checked, and the script header owns the exact threshold and marker rules.
