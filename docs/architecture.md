...
While that acknowledgement is still outstanding the signal is deferred rather than decided - no queue record, and no `.seen-*` marker advance - so it is delivered in full once `FM_PROCEVENT_SIGNAL_DEFER_GRACE` lapses without one.
