...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, heartbeat backstop hits, and a task whose `bin/fm-retry-pressure.sh` reading has reached `retry_band=halt`.
...
In the watcher, only the first escalation of an uninterrupted quiet spell fires at `FM_STALE_ESCALATE_SECS`; each re-escalation of that same spell backs off exponentially up to `FM_WEDGE_ESCALATE_MAX_SECS` and resets to the base threshold once the pane shows genuine activity again, so a lasting wedge stops re-waking the primary model at a fixed cadence.
A quiet spell normally starts at the poll that first sees the pane unchanged, except on a window's first poll by a given watcher process, which seeds the spell from durable idle evidence outside the watcher (the busy-lib Stop record's idle timestamp, else the transcript age from `bin/fm-context-usage.sh`) so a watcher restart never grants an already-idle pane a fresh grace period; that seed is process-local by design, because the on-disk hash baseline persists across restarts and cannot tell a restart from an in-process churn, and any later hash change in the same process drops it so a pane that just churned always restarts from the current time (`wedge_seed_first_observation` in `bin/fm-watch.sh` owns the rule).
A `harness=opencode` pane whose `model=` is a `pc02-llamaswap/*` model takes a raised idle floor (`FM_PC02_STALE_ESCALATE_SECS`) instead of any lower threshold, because that host's local-model turns are normally silent for minutes; before such a lane escalates, the watcher also reads the opencode log (`FM_OPENCODE_LOG`, scanned as a byte-bounded tail sized by `FM_OPENCODE_LOG_TAIL_BYTES`) for a `message=loop ... step=` line newer than the quiet window that carries the task's own recorded session id (`state/<id>.opencode-session`), and a fresh step restarts the quiet-spell timer without burning an escalation.
With no recorded session id there is nothing to attribute a step line to, so the lane escalates on the raised floor alone.
...
A status change that an acknowledged process-event `check` wake already carried is absorbed instead of enqueued a second time, so one underlying event costs one queue record and one drain turn. Absorption is judged on the lines appended since that file's last surfaced signature, and only when every one of them is a line the drain itself reprints (a still-open decision or an unread informational surface); any routine line in that span keeps its own signal wake.
Coverage requires positive proof: the generation's surfaced wake must be strictly newer, at nanosecond resolution, than the status write, the drain must have acknowledged it through `fm-procevent.sh handled`, and the change must be one only the signal annotation would present (an open decision or an unread surface line).
While that acknowledgement is still outstanding a change that could be covered is deferred rather than decided - no queue record, and no `.seen-*` marker advance, in that cycle or in a benign-absorb one that shares it - so it is delivered in full once `FM_PROCEVENT_SIGNAL_DEFER_GRACE` lapses without one; a change no acknowledgement could ever cover, such as a routine line, is never deferred and wakes immediately.
Away mode gates that whole coverage decision: while `state/.afk` exists no signal is classified as deferred or covered at all, so every coverable signal takes the away-mode queue-and-exit-immediately path instead of being delayed for the grace window or absorbed.
A covered file is excluded from the set-wide actionability verdict, so its captain-relevant span never forces a benign sibling in the same scan into the enqueue branch; the covered file then absorbs through the benign branch with its markers advanced, and a failed classified-position commit for it is neither escalated into a wake nor retried on the next cycle - the reported signature advanced in the same pass suppresses the file, bounding the absorb to one per distinct file state, so the classified position is re-attempted only after a new append changes the bytes and the heartbeat backstop covers the stale position in between.
An absorb only ever consumes the bytes its coverage verdict judged: the file's size is read once, alongside that verdict, and a classified endpoint past it - an append that landed later in the same cycle - refuses the absorb, leaving the change to be decided fresh on the next cycle.
An absorbed covered change instead advances both its `.seen-*` markers and the heartbeat backstop's `.hb-surfaced-<task>` marker, so the backstop cannot re-surface the very event that acknowledgement proves was already delivered.
`tests/fm-wake-pair-dedup.test.sh` pins the duplicate pair, the routine-line exemption, a same-second distinct change that must still wake, the bounded deferral, and the untouched no-process-event path.
`tests/fm-watch-triage.test.sh` pins the mixed scan - one covered log beside a benign sibling - that is the only shape reaching that benign-branch absorb, asserting no wake, no queue record, and the covered log's committed classified position; `tests/fm-watch-pc02-cadence.test.sh` pins that byte-scoping predicate directly: an endpoint past the recorded extent, or an unrecorded or unparsable one, refuses.
...
A crew that declares a self-clearing `paused:` wait, or carries a verified `captain-held` transfer, is separately absorbed while idle and re-surfaced only on the longer pause cadence, rather than being treated as a possible wedge.
...
No-change heartbeats are also benign, except when the heartbeat's own fleet scan finds a task at `retry_band=halt`: that reading is named in the wake reason and in the durable wake row, and is surfaced once per reading (again only when the relaunch count moves), because nothing else runs that helper unprompted.
To keep that scan cheap the reader is bounded four ways - at most once per real heartbeat interval, only for tasks with a `state/<id>.meta` record, time-bounded per invocation, and time-bounded across the whole fleet loop so the poll cannot stall past the watcher guard grace.
...
A wedged CI poll - the same CI check command failing identically several times with no forward-progress marker after its last occurrence, where a trailing per-poll heartbeat counts as progress only if no heartbeat is interleaved with those failures - overrides an earlier marker and reports the crew failed, so a poll that can never progress is never read as still monitoring; the coarse cross-branch fallback is not wedge-checked, and the script header owns the exact threshold and marker rules.
...
The watcher and daemon share `bin/fm-classify-lib.sh` for captain-relevant status verbs, declared-wait vocabulary (a self-clearing `paused:` wait and a verified `captain-held` transfer alike, through one combined predicate), and status-scan primitives.
That predicate takes the task's recorded worktree so it can exclude a context-exhausted `paused:` (not keyed `session-limit`, worker context band warn or restart), which is a stop needing relaunch rather than a wait; `bin/fm-classify-lib.sh`'s `status_pause_needs_action` owns that discriminator and `bin/fm-crew-state.sh` reports such a line as `blocked`.
...
An ordinary local text steer is first subject to the prompt-cache steer guard, which refuses a session idle past the cache TTL and names the relaunch command instead ([`configuration.md`](configuration.md) "Prompt-cache steer guard" owns that policy and its override).
...
PC02's llama-swap serves one model at a time, so `fm-spawn.sh` refuses any spawn or relaunch onto a `pc02-llamaswap/*` model while another task's meta holds one whose endpoint is not positively dead or missing, naming the occupying task so dispatch falls through to the rule's next candidate; ambiguous liveness, including any remote task's lane, deliberately keeps the lane occupied, and the check is scoped to the spawning home's own task set.
...
That keeps spawn launch compatible across claude, opencode, pi, pi-signed, grok, kimi, cursor, gemini, muse, rovo, and omp while preserving the requested profile for later audit.
...
`config/crew-harness` remains the crewmate harness and is inherited into secondmate homes, unless a secondmate home pins its own value with a sibling `config/crew-harness.local-override` marker ([`secondmate-provisioning`](../.agents/skills/secondmate-provisioning/SKILL.md) owns that exception).
...
`bin/fm-project-mode.sh` remains the one registry parser for the mechanical consumers that have no task in hand: fleet sync's `local-only` skip, home seeding's refusal and no-mistakes initialization, and the queue snapshot's read-only inference of a repo-less backlog item's project from its `-`-delimited id prefix (`--infer-project-from-id`), which never writes that inference back to the item.
