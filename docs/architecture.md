...
`tests/fm-watch-triage.test.sh` pins the mixed scan - one covered log beside a benign sibling - that is the only shape reaching that benign-branch absorb, asserting no wake, no queue record, and the covered log's committed classified position.
