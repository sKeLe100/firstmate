...
A covered file is excluded from the set-wide actionability verdict, so its captain-relevant span never forces a benign sibling in the same scan into the enqueue branch; the covered file then absorbs through the benign branch with its markers advanced, and a failed marker commit for it is retried on the next cycle rather than escalated into a wake.
