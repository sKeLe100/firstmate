...
To keep that scan cheap the reader is bounded three ways - at most once per real heartbeat interval, only for tasks with a `state/<id>.meta` record, and time-bounded per invocation.
