...
A wedged CI poll - the same CI check command failing identically several times with no forward-progress marker after its last occurrence - overrides those markers and reports the crew failed, so a poll that can never progress is never read as still monitoring; the script header owns the exact threshold and marker rules.
