...
A status change that an acknowledged process-event `check` wake already carried is absorbed instead of enqueued a second time, so one underlying event costs one queue record and one drain turn.
