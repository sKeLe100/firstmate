...
Neither the title nor the row is ever rewritten. The mark is written once and never reset by a later re-hold, marks for tasks no longer in `backlog.md` are pruned lazily on each write, and `bin/fm-bearings-snapshot.sh` exposes it read-only as `decisions_open.since`/`created_at` - falling back to the row's tasks-axi creation date for rows that predate the sidecar - which is what ages a decision for the autonomous 48h threshold.
