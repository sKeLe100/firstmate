...
On the first hold, `hold` also records a machine-owned `held-since <UTC-ISO8601>` mark for the task in firstmate's own sidecar `data/task-marks.tsv` (`<task-id>\t<key>\t<value>`, the same shape as `data/roundtable-marks.tsv`).
The mark lives in the sidecar rather than the task row because tasks-axi owns the row's trailing metadata block: it already writes its own `since` word (the task's creation date) and its parser rejects any word it does not know.
Neither the title nor the row is ever rewritten. The mark is not reset by a later re-hold, but closing the call clears it - `answer --release` drops the task's `held-since` so a later re-hold restamps from the new call, and a `done` close drops every mark for the task - and marks for tasks no longer in `backlog.md` are pruned lazily on each write.
`bin/fm-bearings-snapshot.sh` exposes the mark read-only as `decisions_open.since`/`created_at` - falling back to the row's tasks-axi creation date for rows that predate the sidecar - which is what ages a decision for the autonomous 48h threshold.
The `mark set|get|clear` subcommands are the sidecar's only writer, shared with the autonomous pass's `deferred-since` mark (surfaced as `gates[].deferred_since`); each write takes `data/.task-marks.lock` around its whole-file rewrite, `set` never overwrites an existing value and refuses an empty one, and `clear` without a key drops every mark for the task.
