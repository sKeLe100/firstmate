...
On the first hold, `hold` also writes a machine-owned `since <UTC-ISO8601>` metadata word into the task row's trailing backlog metadata block - the same space-separated syntax as `merged`/`reported`/`done`, alongside the colon-style `repo:`/`kind:`/`hold:`/`hold-kind:` fields, and folded into that block only when it already starts with a known metadata key so a title ending in parentheses is never mutated.
The title itself is never rewritten. The word is written once and never reset by a later re-hold, and `bin/fm-bearings-snapshot.sh` exposes it read-only as `decisions_open.since`/`created_at`, which is what ages a decision for the autonomous 48h threshold.
