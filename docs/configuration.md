...
The optional `<state-dir> <task-id>` pair - the CALLER's own state directory and the task or secondmate id, given together or not at all - feeds `bin/fm-cache-ttl-lib.sh`'s shared activity-age fold into an `idle_seconds` field on the same output line, alongside `ttl_seconds` resolved from `config/cache-ttl-seconds` under `<home-path>`.
