...
The optional local, gitignored `config/upstream-autosync` presence flag additionally gates auto-dispatch eligibility: with it present, `fm-upstream-sync-item.sh` reports `eligible=yes` once the fork is `FM_UPSTREAM_AUTOSYNC_COMMIT_THRESHOLD` commits behind (default 5) or `FM_UPSTREAM_AUTOSYNC_DAYS_THRESHOLD` days behind (default 7, the weekly cadence); with it absent, or below both thresholds, it reports `eligible=no` and the item stays queued for the captain exactly as it does today.
The same flag gives the sync its schedule: with it present, `bin/fm-bootstrap.sh` arms `state/upstream-drift.check.sh` once per home (the same idempotent arming as `bin/fm-upstream-behind-check.sh arm`) so every watcher poll drives the once-daily drift check, and its deferred network stage runs that check itself once per session start, so a home whose fleet was idle with no watcher still checks on the daily cadence.
Nothing else needs a cron entry or a separate scheduler; an eligible item is then dispatched by the ordinary heartbeat refill of idle capacity (AGENTS.md section 8) like any other queued work.
Without the flag nothing is armed or polled by bootstrap, and a check armed by hand is left alone.

Each dispatched sync is a bounded batch, never the whole gap.
`bin/fm-upstream-batch.sh plan` cuts the pending upstream delta at its first-parent line into the next `FM_UPSTREAM_AUTOSYNC_BATCH_MAX` commits (default 20) and names the exact upstream commit that batch merges up to; the commits after it wait for the next periodic sync.
The filed item's note carries that target, and `bin/fm-brief.sh --upstream-sync` plans it again at scaffold time and embeds it as the worker's bounded-batch gate, so the bound never depends on the dispatcher remembering a flag.
A batch is always taken with a true merge commit of that target - never a rebase of the fork's own branch, never a squash, never past the target - because a rebase replays every fork commit through every upstream change and is exactly the needless conflict surface the bound exists to avoid.
The batch gate also draws the mechanical-versus-real conflict line: a conflict whose resolution is unambiguous and content-preserving is resolved in a labeled conflict-resolution commit, while any conflict that needs a judgment about which side's intent wins is held as a keyed captain decision with the same three-way summary as the supervision-safety stop, and an ask-user finding raised by the pipeline during a sync is escalated through `ask-user-authority` like any other rather than answered by the worker.
...
The regression-stop boundary is applied with a tracked quarantine of known pre-existing, host-sensitive test failures, so a failure that already exists on the fork's own `main` cannot stall every sync batch.
The list lives at `tests/fm-test-quarantine.tsv` - one entry per line, each naming a test script, its failure signature, the host-sensitivity reason (with its evidence), and the backlog item that owns fixing and thereby removing it - and is read by `bin/fm-test-run.sh --exclude-quarantined`, which drops exactly those scripts from a selection and prints one loud `FM_TEST_QUARANTINED` line per skip plus an `FM_TEST_QUARANTINE_SUMMARY` line, so a quarantined skip is never silent.
...
`bin/fm-cache-ttl-lib.sh` is the one implementation of that knob and of the activity-age fold, shared by the guard, the heartbeat flag, and `bin/fm-returning-session-check.sh`'s idle dimension so they cannot drift apart.
...
`bin/fm-lint.sh` guards this canonical `commands.test` value on every no-mistakes lint run and fails loudly if the tracked file drifts from it, so a per-branch workaround can no longer land silently as standing policy; the check's exact mechanics are owned by that script's own header comment.
It raises `test_agent_timeout` to `60m` from the 30m default because that pinned invocation can outrun 30m under host load; the file's own comment owns that rationale.
...
The optional `<state-dir> <task-id>` pair - the CALLER's own state directory and the task or secondmate id, given together or not at all - feeds `bin/fm-cache-ttl-lib.sh`'s shared activity-age fold into an `idle_seconds` field on the same output line, alongside `ttl_seconds` resolved from `cache-ttl-seconds` in the caller's own `config/` directory beside that `<state-dir>` (the same knob `fm-send.sh`, `fm-fleet-snapshot.sh`, and `fm-inactive-reconcile.sh` read); with the pair omitted there is no marker owner to read that knob from and idle can never affect the verdict, so `ttl_seconds` reports `n/a`.
The verdict is `restart-with-carryover` when `band=restart`, OR when `band=warn` and the session has sat idle past that TTL: a warn-band session gone cold pays the same return-tax re-read a restart-band session does, so a bare resume just walks it into the restart band on the next turn anyway.
`band=ok` never restarts on idle alone, and an unresolvable idle (`idle_seconds=unknown` - the pair omitted, or no readable activity marker yet) or a disabled TTL (`ttl_seconds<=0`) never contributes to the verdict, leaving the original context-only behavior exactly as it was before this field existed.
...
A bare `codex` pin is the exception and no longer spawns: codex requires an explicit model and an accepted effort on this line (or on the spawn) because Codex CLI bills its own default when a flag is absent ([architecture.md](architecture.md#dispatch-profiles)).
...
The essential universal toolchain is node, git, gh v2.63.0 or newer with GitHub auth via `gh auth login`, no-mistakes v1.46.0 or newer, compatible gh-axi, chrome-devtools-axi, compatible tasks-axi per "Backlog backend" above, and compatible quota-axi.
[`bin/fm-bootstrap.sh`](../bin/fm-bootstrap.sh) owns the axi-family floor policy and the gh, gh-axi, and lavish-axi floors, while [`bin/fm-tasks-axi-lib.sh`](../bin/fm-tasks-axi-lib.sh) and [`bin/fm-quota-axi-lib.sh`](../bin/fm-quota-axi-lib.sh) hold their own tools' floor constants.
...
FM_WSL_CONFIG_PATH=      # override .wslconfig path checked by the WSL2 mirrored-networking hint in fm-remote-readiness-lib.sh, tests only
FM_WSL_USERS_DIR=        # override /mnt/c/Users base dir used to resolve that per-user .wslconfig, tests only
...
FM_UPSTREAM_AUTOSYNC_DAYS_THRESHOLD=7      # age in days of the oldest unmerged upstream commit that makes that item eligible on the same gate (the weekly cadence)
FM_UPSTREAM_AUTOSYNC_BATCH_MAX=20          # first-parent upstream commits one sync task may merge; bin/fm-upstream-batch.sh plans the exact target and the rest waits for the next periodic sync
