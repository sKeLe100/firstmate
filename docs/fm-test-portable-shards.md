...
The check also requires the measured duration to exceed its hint by at least `PORTABLE_SERIAL_HINT_DRIFT_FLOOR_MS` (1000ms) before counting the ratio at all, so a ~50ms script crossing 1.5x on a ~35ms swing is scheduler noise, not drift; the floor leaves the ratio meaningful for the longer scripts it was designed to catch. `--hint-drift-history` persists this run's drift set to `<path>` and compares it against what was persisted there on the previous run; the check refuses only when the *same* script drifts on two consecutive runs, so a genuine, sustained regression still gets caught while a lone noisy run no longer produces a false red. Without `--hint-drift-history` nothing can ever be confirmed twice, so the check can only warn.
