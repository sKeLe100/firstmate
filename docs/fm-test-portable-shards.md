...
A wedged script inside the portable-serial lane can consume the whole job budget with zero output before the job-level cap cancels it, leaving no evidence of which script or why (see the per-script bound above). The suspected mechanism is a rare bash async-signal/trap race in `bin/fm-wake-lib.sh`'s lock-wait/handoff path (`_fm_lock_acquire_wait_handoff`, `fm_lock_acquire_wait_bounded`) under external process-group kill; it has not been reproduced locally and `fm-wake-lib.sh` is safety-critical, so no fix has been attempted there pending real reproduction.
