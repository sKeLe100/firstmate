...
A script that drifts on only one run is logged as a `::warning::`, not a failure - shared-runner noise can swing a single ordinary green run past the margin without the hint actually being stale (this is what forced the multiplier to 2x for a while; see git history).
The check also requires the measured duration to exceed its hint by at least `PORTABLE_SERIAL_HINT_DRIFT_FLOOR_MS` (250ms) before counting the ratio at all, so a ~50ms script crossing 1.5x on a ~30ms swing (51->88ms, 54->82ms) is scheduler noise, not drift; 250ms absorbs that noise while a script genuinely drifting in the 1-2s range (e.g. 1106->2985ms) still flags. `--hint-drift-history` persists this run's drift set to `<path>` and compares it against what was persisted there on the previous run; the check refuses only when the *same* script drifts on two consecutive runs, so a genuine, sustained regression still gets caught while a lone noisy run no longer produces a false red. Without `--hint-drift-history` nothing can ever be confirmed twice, so the check can only warn.
