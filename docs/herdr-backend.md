...
A structurally gone pane or a pane read from a session positively reported as having no running server becomes `missing`, a restored agent-less shell, a husk registration proven dead by its bare idle shell, and a stale registration over a shell-only pane all become `dead`, a registered agent with a live process becomes `alive`, and every other unexpected read becomes `unreadable`.
...
### Mid-poll pane_not_found recovery
