...
| `fm-primary-scope-lib.sh` | Shared location-keyed marker-or-plain-checkout primary-home predicate for tracked hooks |
...
| `fm-retry-pressure.sh`   | Report a task's retry-loop pressure band from its relaunch telemetry and worker-reported retry-loop status line |
...
| `fm-autonomous-thresholds.sh` | Evaluate the `/autonomous` dispatch pass's bundle-size and time nudge thresholds over a JSON array of open decision rows |
