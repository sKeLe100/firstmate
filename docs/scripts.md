...
| `fm-orphan-pane-sweep.sh` | Bounded, read-only detection of herdr panes outside this home's task tracking; reports only, never closes |
...
| `fm-agentsmd-size.sh`   | Assert this repo's `AGENTS.md` stays under its resident-size ceiling, which the script's own header and constant own |
...
| `fm-gemini-guard-lib.sh` | One owner of the deterministic Gemini dispatch and quota-safety guard: model allowlist, input-size bound, session cap, and before/after launch accounting |
...
| `fm-supervisor-target-lib.sh` | Resolve and validate the shared supervisor target and backend for the daemon and launcher |
