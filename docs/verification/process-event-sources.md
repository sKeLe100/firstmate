...
| one event, one wake | a captured result's `check` wake and the same task's status write are never enqueued as two records: with the generation acknowledged, the covered signal is absorbed without waking; a routine status line is never absorbed; a distinct status change written after the surfaced wake, in the same epoch second, still wakes; an unacknowledged generation only defers the signal, which is delivered in full once `FM_PROCEVENT_SIGNAL_DEFER_GRACE` lapses; a status signal with no process-event present wakes unchanged; and a covered status change sharing a scan with a benign sibling is absorbed on the benign path with its classified position committed, still without a queue record or wake; and while `state/.afk` exists a coverable signal is neither deferred nor absorbed but queued with the wake exiting immediately (`tests/fm-watch-triage.test.sh`) |
