...
Its OpenCode positive lock-held arm fixture is currently quarantined out of the invocation list because that fixture's login-shell arm spawn reproducibly hangs on hosted CI runners; the negative no-arm-without-lock half still runs, and the hang is tracked as backlog item `pi-watch-arm-spawn-hang-investigation`.
