...
test_perspective_flag_inserts_catalog_fragment_and_marker() {
  ...
  FM_HOME="$home" "$ROOT/bin/fm-brief.sh" brief-persp-ship firstmate --mode no-mistakes --perspective architect >/dev/null 2>&1 \
    || fail "ship --perspective architect refused"
  brief="$home/data/brief-persp-ship/brief.md"
  assert_grep "Perspective: architect" "$brief" "ship brief lacks the Perspective: line"
...
test_perspective_flag_refuses_unknown_or_unsafe_slug() {
  ...
  out=$(FM_HOME="$home" "$ROOT/bin/fm-brief.sh" brief-persp-review-ship firstmate --mode no-mistakes --perspective independent-reviewer 2>&1); rc=$?
  [ "$rc" -ne 0 ] || fail "a review perspective was accepted on a ship brief"
  printf '%s' "$out" | grep -F "applies only to scout briefs" >/dev/null || fail "review-on-ship refusal missing: $out"
  assert_absent "$home/data/brief-persp-review-ship/brief.md" "ship brief was written despite a review perspective"
  ...
  pass "fm-brief.sh: --perspective refuses unknown, empty, unsafe, review-on-ship, and charter-scoped slugs"
}
