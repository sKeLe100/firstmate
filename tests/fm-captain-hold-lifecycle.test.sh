...
# Done retention (tasks-axi prune) moves resolved captain-held tasks out of
# data/backlog.md into data/done-archive.md, which tasks-axi's own `show`
# cannot read back. A durably answered task must keep passing verify (and
# therefore scout teardown) after that move, never wedge teardown just
# because retention already ran.
test_verify_accepts_archived_answered_hold() {
  ...
  run_captain "$home" hold sample-archive-call \
    --title "Choose the archive sample outcome" --reason "captain archive choice pending" --repo sample >/dev/null \
    || fail "could not register the captain-held task for the archive scenario"
  run_captain "$home" complete "$id" sample-archive-call >/dev/null \
    || fail "completion failed before the answer landed"
  ...
  run_captain "$home" answer sample-archive-call --decision-file "$home/archive-decision.txt" >/dev/null \
    || fail "could not answer the archive-scenario captain-held task"

  run_captain "$home" verify "$id" >/dev/null \
    || fail "verify refused an answered task still present in the live backlog"

  tasks_in "$home" prune --state "done" --keep 0 >/dev/null \
    || fail "could not archive the answered task via Done retention"
  ! grep -E "^- \[x\] sample-archive-call -" "$home/data/backlog.md" >/dev/null \
    || fail "answered task fixture was not pruned out of the live backlog"
  grep -E "^- \[x\] sample-archive-call -" "$home/data/done-archive.md" >/dev/null \
    || fail "answered task fixture was not archived"

  run_captain "$home" verify "$id" >/dev/null \
    || fail "verify wrongly refused an answered task archived by Done retention"
  ...
  pass "verify accepts an answered captain-held task archived by Done retention"
}

# A hold that decayed out of its durable held state (unheld without ever being
# closed through fm-captain-hold.sh answer) must keep failing verify exactly
# as before the archive fallback was added - the archive lookup must never
# paper over a genuinely open, improperly tracked captain call.
test_verify_rejects_unheld_unresolved_hold() {
  ...
  run_captain "$home" hold sample-decay-call \
    --title "Choose the decay sample outcome" --reason "captain decay choice pending" --repo sample >/dev/null \
    || fail "could not register the captain-held task for the decay scenario"
  run_captain "$home" complete "$id" sample-decay-call >/dev/null \
    || fail "completion failed while the task was actively held"

  tasks_in "$home" unhold sample-decay-call >/dev/null || fail "could not unhold the decay-scenario fixture"
  if run_captain "$home" verify "$id" > "$home/decay-verify.out" 2> "$home/decay-verify.err"; then
    fail "verify accepted an unheld, unresolved captain-held task"
  fi
  assert_grep "neither held for the captain nor closed with a recorded captain answer" "$home/decay-verify.err" \
    "decayed hold must fail with the unchanged not-held-not-answered diagnostic"
...
test_verify_rejects_hold_missing_from_both_surfaces() {
  ...
  if run_captain "$home" verify "$id" > "$home/ghost-verify.out" 2> "$home/ghost-verify.err"; then
