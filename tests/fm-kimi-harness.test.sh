...
BASE_PATH=${FM_TEST_BASE_PATH:-$PYTHON_BIN_DIR:$(fm_test_base_path)}
...
test_kimi_launch_then_send_is_verified() {
  ...
  [ "$launch" = "env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI -u FM_CREW_STATE_META_OVERRIDE -u FM_CREW_STATE_STATUS_OVERRIDE '$FAKEBIN_DIR/kimi' --model 'kimi-code/k3' --auto" ] \
...
test_kimi_falls_back_to_expanded_home_binary() {
  ...
  launch=$(cat "$CASE_DIR/launch.log")
  [ "$launch" = "env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI -u FM_CREW_STATE_META_OVERRIDE -u FM_CREW_STATE_STATUS_OVERRIDE '$fallback' --auto" ] \
