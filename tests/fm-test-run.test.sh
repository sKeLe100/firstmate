...
# selection, portable lane composition, proven-isolated --jobs, bounded
# concurrency, opt-in --fail-fast, budgets, timing markers, JSON artifacts,
# coverage guard, and aggregate exit status.
#
...
test_check_hint_drift_flags_stale_hint() {
  local tmp script hint clean_json drift_json drift2_json rc out
...
JSON
  out=$("$RUNNER" --check-hint-drift "$clean_json") \
  ...
  assert_contains "$out" "drift=0" "clean run must report zero drift"
  ...
  # rule the hint table itself is built on.
...
JSON
  set +e
  out=$("$RUNNER" --check-hint-drift "$drift_json" "$drift2_json" 2>&1)
  rc=$?
  set -e
  [ "$rc" -ne 0 ] || fail "a measured duration past 1.5x the hint must refuse: $out"
  ...
  rm -rf "$tmp"
  pass "--check-hint-drift refuses a stale hint and stays clean at or near the recorded value"
}
